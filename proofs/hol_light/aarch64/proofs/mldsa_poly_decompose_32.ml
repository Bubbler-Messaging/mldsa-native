...
(**** print_literal_from_elf "aarch64/mldsa/mldsa_poly_decompose_32.o";;
 ****)

let mldsa_poly_decompose_32_mc = define_assert_from_elf "mldsa_poly_decompose_32_mc" "aarch64/mldsa/mldsa_poly_decompose_32.o"
(*** BYTECODE START ***)
[
...
let MLDSA_POLY_DECOMPOSE_32_EXEC = ARM_MK_EXEC_RULE mldsa_poly_decompose_32_mc;;
...
let LENGTH_MLDSA_POLY_DECOMPOSE_32_MC =
  REWRITE_CONV[mldsa_poly_decompose_32_mc] `LENGTH mldsa_poly_decompose_32_mc`
...
let MLDSA_POLY_DECOMPOSE_32_CORE_END = new_definition
  `MLDSA_POLY_DECOMPOSE_32_CORE_END =
     LENGTH mldsa_poly_decompose_32_mc - MLDSA_POLY_DECOMPOSE_32_POSTAMBLE_LENGTH`;;
...
(* Connect word-level decompose32_a1/a0 to spec-level decompose32           *)
(* ========================================================================= *)
...
(* decompose32_a1 computes FST(decompose32(val x)) *)
let DECOMPOSE32_A1_CORRECT = prove(
  `!x:int32. val x < 8380417
    ==> val(decompose32_a1 x) = FST(decompose32(val x))`,
  GEN_TAC THEN DISCH_TAC THEN
  REWRITE_TAC[DECOMPOSE32_A1_CASES; DECOMPOSE32_EXPAND; LET_DEF; LET_END_DEF; FST] THEN
  COND_CASES_TAC THENL [
...
(* cmod n 523776 is bounded by [-261888, 261888], well within int32 range *)
let CMOD_ABS_BOUND_523776 = prove(
  `!n. abs(cmod n 523776) <= &261888`,
  GEN_TAC THEN REWRITE_TAC[cmod] THEN
...
(* decompose32_a0 computes SND(decompose32(val x)) *)
let DECOMPOSE32_A0_CORRECT = prove(
  `!x:int32. val x < 8380417
    ==> ival(decompose32_a0 x) = SND(decompose32(val x))`,
  GEN_TAC THEN DISCH_TAC THEN
  REWRITE_TAC[DECOMPOSE32_A0_CASES; DECOMPOSE32_EXPAND; LET_DEF; LET_END_DEF; SND] THEN
  ...
  (* Prove key identity: val x - h * 523776 = cmod(val x) 523776 *)
  SUBGOAL_THEN `&(val(x:int32)) - &h * &523776 = cmod (val x) 523776`
    ASSUME_TAC THENL [
    REWRITE_TAC[cmod] THEN
  ...
  COND_CASES_TAC THENL [
    ...
    SUBGOAL_THEN `word_sub (iword(cmod (val(x:int32)) 523776)) (word 1) : int32 =
      iword(cmod (val x) 523776 - &1)` SUBST1_TAC THENL [
...
let MLDSA_POLY_DECOMPOSE_32_CORRECT = prove(
 `!pc a r1 x.
    nonoverlapping (word pc, LENGTH mldsa_poly_decompose_32_mc)
                   (r1, 1024) /\
    nonoverlapping (word pc, LENGTH mldsa_poly_decompose_32_mc)
                   (a, 1024) /\
    ...
    ==> ensures arm
         (\s. aligned_bytes_loaded s (word pc) mldsa_poly_decompose_32_mc /\
              ...
              (!i. i < 256
                   ==> val(read(memory :> bytes32
                     (word_add r1 (word(4 * i)))) s) =
                       FST(decompose32(val(x i)))) /\
              (!i. i < 256
                   ==> ival(read(memory :> bytes32
                     (word_add a (word(4 * i)))) s) =
                       SND(decompose32(val(x i)))))
...
let MLDSA_POLY_DECOMPOSE_32_SUBROUTINE_CORRECT = prove(
 `!pc a r1 x returnaddress.
              ...
              (!i. i < 256
                   ==> ival(read(memory :> bytes32
                     (word_add a (word(4 * i)))) s) =
                       SND(decompose32(val(x i)))) /\
              (!i. i < 256
                   ==> FST(decompose32(val(x i))) <= 15) /\
              (!i. i < 256
                   ==> --(&261888) <= SND(decompose32(val(x i))) /\
                       SND(decompose32(val(x i))) <= &261888))
         (MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
          MAYCHANGE [memory :> bytes(r1, 1024)] ,,
          MAYCHANGE [memory :> bytes(a, 1024)])`,
  CONV_TAC LENGTH_SIMPLIFY_CONV THEN
  ...
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  CONJ_TAC THENL [
    REPEAT STRIP_TAC THEN MATCH_MP_TAC DECOMPOSE32_A1_BOUND THEN
    FIRST_X_ASSUM MATCH_MP_TAC THEN ASM_REWRITE_TAC[];
    GEN_TAC THEN DISCH_TAC THEN MATCH_MP_TAC DECOMPOSE32_A0_BOUND THEN
    FIRST_X_ASSUM MATCH_MP_TAC THEN ASM_REWRITE_TAC[]]);;
