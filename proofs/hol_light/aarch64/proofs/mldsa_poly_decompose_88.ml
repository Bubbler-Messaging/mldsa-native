...
(**** print_literal_from_elf "aarch64/mldsa/mldsa_poly_decompose_88.o";;
 ****)

let mldsa_poly_decompose_88_mc = define_assert_from_elf "mldsa_poly_decompose_88_mc" "aarch64/mldsa/mldsa_poly_decompose_88.o"
(*** BYTECODE START ***)
[
...
let MLDSA_POLY_DECOMPOSE_88_EXEC = ARM_MK_EXEC_RULE mldsa_poly_decompose_88_mc;;
...
let LENGTH_MLDSA_POLY_DECOMPOSE_88_MC =
  REWRITE_CONV[mldsa_poly_decompose_88_mc] `LENGTH mldsa_poly_decompose_88_mc`
...
let MLDSA_POLY_DECOMPOSE_88_CORE_END = new_definition
  `MLDSA_POLY_DECOMPOSE_88_CORE_END =
     LENGTH mldsa_poly_decompose_88_mc - MLDSA_POLY_DECOMPOSE_88_POSTAMBLE_LENGTH`;;
...
(* Connect word-level decompose88_a1/a0 to spec-level decompose88           *)
(* ========================================================================= *)
...
(* decompose88_a1 computes FST(decompose88(val x)) *)
let DECOMPOSE88_A1_CORRECT = prove(
  `!x:int32. val x < 8380417
    ==> val(decompose88_a1 x) = FST(decompose88(val x))`,
  GEN_TAC THEN DISCH_TAC THEN
  REWRITE_TAC[DECOMPOSE88_A1_CASES; DECOMPOSE88_EXPAND; LET_DEF; LET_END_DEF; FST] THEN
  COND_CASES_TAC THENL [
...
(* cmod n 190464 is bounded by [-95232, 95232], well within int32 range *)
let CMOD_ABS_BOUND_190464 = prove(
  `!n. abs(cmod n 190464) <= &95232`,
...
(* decompose88_a0 computes SND(decompose88(val x)) *)
let DECOMPOSE88_A0_CORRECT = prove(
  `!x:int32. val x < 8380417
    ==> ival(decompose88_a0 x) = SND(decompose88(val x))`,
  GEN_TAC THEN DISCH_TAC THEN
  REWRITE_TAC[DECOMPOSE88_A0_CASES; DECOMPOSE88_EXPAND; LET_DEF; LET_END_DEF; SND] THEN
  ...
  (* Prove key identity: val x - h * 190464 = cmod(val x) 190464 *)
  SUBGOAL_THEN `&(val(x:int32)) - &h * &190464 = cmod (val x) 190464`
    ASSUME_TAC THENL [
  ...
  COND_CASES_TAC THENL [
    ...
    SUBGOAL_THEN `word_sub (iword(cmod (val(x:int32)) 190464)) (word 1) : int32 =
      iword(cmod (val x) 190464 - &1)` SUBST1_TAC THENL [
...
let MLDSA_POLY_DECOMPOSE_88_CORRECT = prove(
 `!pc a r1 x.
    nonoverlapping (word pc, LENGTH mldsa_poly_decompose_88_mc)
                   (r1, 1024) /\
    nonoverlapping (word pc, LENGTH mldsa_poly_decompose_88_mc)
                   (a, 1024) /\
    ...
    ==> ensures arm
         (\s. aligned_bytes_loaded s (word pc) mldsa_poly_decompose_88_mc /\
              ...
              (!i. i < 256
                   ==> val(read(memory :> bytes32
                     (word_add r1 (word(4 * i)))) s) =
                       FST(decompose88(val(x i)))) /\
              (!i. i < 256
                   ==> ival(read(memory :> bytes32
                     (word_add a (word(4 * i)))) s) =
                       SND(decompose88(val(x i)))))
...
let MLDSA_POLY_DECOMPOSE_88_SUBROUTINE_CORRECT = prove(
 `!pc a r1 x returnaddress.
              ...
              (!i. i < 256
                   ==> ival(read(memory :> bytes32
                     (word_add a (word(4 * i)))) s) =
                       SND(decompose88(val(x i)))) /\
              (!i. i < 256
                   ==> FST(decompose88(val(x i))) <= 43) /\
              (!i. i < 256
                   ==> --(&95232) <= SND(decompose88(val(x i))) /\
                       SND(decompose88(val(x i))) <= &95232))
         (MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
          MAYCHANGE [memory :> bytes(r1, 1024)] ,,
          MAYCHANGE [memory :> bytes(a, 1024)])`,
  CONV_TAC LENGTH_SIMPLIFY_CONV THEN
  ...
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  CONJ_TAC THENL [
    REPEAT STRIP_TAC THEN MATCH_MP_TAC DECOMPOSE88_A1_BOUND THEN
    FIRST_X_ASSUM MATCH_MP_TAC THEN ASM_REWRITE_TAC[];
    GEN_TAC THEN DISCH_TAC THEN MATCH_MP_TAC DECOMPOSE88_A0_BOUND THEN
    FIRST_X_ASSUM MATCH_MP_TAC THEN ASM_REWRITE_TAC[]]);;
